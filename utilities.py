...
def download_cmi(yyyymmddhhmn, band, path_dest, logging):
...
def download_prod(yyyymmddhhmn, product_name, path_dest, logging):
    os.makedirs(path_dest, exist_ok=True)
    ...
    if 'Contents' not in s3_result:
        ...
        logging.info(f'No files found for the date: {yyyymmddhhmn}, Product-{product_name}')
        return -1
    else:
        # There are files
        for obj in s3_result['Contents']:
