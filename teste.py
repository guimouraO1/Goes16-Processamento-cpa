...
def process_ndvi(ndvi_diario, ch02, ch03, v_extent):
    ...
    # Salvando array NDVI
    NDVI.dump(f'{dir_in}ndvi/ndvi_{date_file}_{v_extent}.npy')
    ...
    if ndvi_diario and datetime.datetime.now().isoweekday() == 6:
        ...
        NDVI_fmin = np.zeros(teste.shape)
        del teste
        ...
        for f in ndvi_list:
            ...
            if date_ini <= date_file <= date_end + datetime.timedelta(minutes=1):
                ...
                NDVI_fmin = np.fmin(NDVI_fmin, NDVI_file)
        ...
        for i in range(0, NDVI_fmax.shape[0]):
            for j in range(0, NDVI_fmax.shape[1]):
                if not -1 < NDVI_fmax[i][j] < 1:
                    NDVI_fmax[i][j] = 0
                if not -1 < NDVI_fmin[i][j] < 1:
                    NDVI_fmin[i][j] = 0

        NDVI_fmax.dump(f'{dir_in}ndvi/ndvi_{date_ini.strftime("%Y%m%d")}_{date_end.strftime("%Y%m%d")}_br_fmax.npy')
        NDVI_fmin.dump(f'{dir_in}ndvi/ndvi_{date_ini.strftime("%Y%m%d")}_{date_end.strftime("%Y%m%d")}_br_fmin.npy')
        ...
        ndvi_list_fmin = [name for name in os.listdir(f'{dir_in}ndvi') if os.path.isfile(os.path.join(f'{dir_in}ndvi', name)) and re.match('^ndvi_.+_.+_br_fmin.npy$', name)]
        # Ordena de forma alfabetica a lista, ficando assim os arquivos mais antigos no comeco
        ndvi_list_fmax.sort()
        ndvi_list_fmin.sort()
        ...
        for f in range(0, len(ndvi_list_fmax)):
            ...
            if date_file_i == date_file_e and date_ini <= date_file_i <= date_end:
                ...
                NDVI_file_fmin = np.load(f'{dir_in}ndvi/{ndvi_list_fmin[f]}', allow_pickle=True)
                # print(ndvi_list_fmax[f], ndvi_list_fmin[f], date_file_i)
                # Compara os arrays NDVI e retorna um novo array com os maiores valores de cada elemento
                NDVI_fmax = np.fmax(NDVI_fmax, NDVI_file_fmax)
                NDVI_fmin = np.fmin(NDVI_fmin, NDVI_file_fmin)

                # Remove o arquivo NDVI
                # os.remove(f'{dir_in}ndvi/{ndvi_list_fmax[f]}')
                # os.remove(f'{dir_in}ndvi/{ndvi_list_fmin[f]}')
            else:
                continue
        ...
        for i in range(0, NDVI_fmax.shape[0]):
            for j in range(0, NDVI_fmax.shape[1]):
                # print(ndvi_list_fmax[f].replace("_fmax.npy", ".npy"), i, j)
                media = (NDVI_fmax[i][j] + NDVI_fmin[i][j]) / 2
                if media > 0.1:
                    NDVI[i][j] = NDVI_fmax[i][j]
                elif media < 0.1:
                    NDVI[i][j] = NDVI_fmin[i][j]
                else:
                    NDVI[i][j] = media

        NDVI.dump(f'{dir_in}ndvi/ndvi_{date_ini.strftime("%Y%m%d")}_{date_end.strftime("%Y%m%d")}_br.npy')
        ...
        # Plotando a imagem
        # img = ax.imshow(NDVI, origin='upper', vmin=-1, vmax=1, cmap='rainbow', extent=img_extent)
        img = ax.imshow(NDVI, origin='upper', vmin=-1, vmax=1, cmap=my_cmap, extent=img_extent)
...
def process_fdcf(fdcf, ch01, ch02, ch03, v_extent, fdcf_diario):
    ...
    grid = remap(fdcf, file_var, extent, resolution)
    ...
    selected_fires = (data == 10) | (data == 11) | (data == 13) | (data == 30) | (data == 33)
    print(selected_fires.shape)
    lat, lon = degrees(fire_mask)
    
    # separando Latitudes e Longitudes dos pontos
    p_lat = lats[selected_fires]
    p_lon = lons[selected_fires]
